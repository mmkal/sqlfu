...
The rule compares the current `.sql` file with the source hash emitted in `.generated/queries.ts`. If the manifest is missing, the source query is missing from it, the wrapper is missing, or the hash no longer matches, lint reports that `sqlfu generate` needs to be run.
