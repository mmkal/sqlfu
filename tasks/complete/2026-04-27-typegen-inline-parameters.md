...
# Typegen query annotations and inline parameter modifiers

## Status (for humans)

Implementation complete on `typegen-pgtyped-support`. `sqlfu generate` keeps unannotated single-query files working, supports `@name` multi-query `.sql` files, expands inline `:ids:list` / `:posts:tupleList(...)` params at runtime, groups `:post.slug` dot-path params into typed objects, emits validator schemas for expanded params, records annotated query entries in the catalog, and documents the feature. Focused typegen tests and typecheck pass.
...
- inline parameter modifiers and dot paths for runtime placeholder expansion
...
/** @name listPostsByIds */
select id, slug from posts where id in (:ids:list);
...
- [x] Add parameter expansion support for scalar arrays: `:ids:list` rewrites into a runtime placeholder list and types the param as an array of the inferred scalar type. _Covered by `listPostsByIds` fixture/runtime assertions._
...
- [x] Add docs for multi-query files and inline parameter modifiers. _Added `packages/sqlfu/docs/typegen.md` and linked it from `packages/sqlfu/README.md` / website sidebar._
...
- This task intentionally does not implement PgTyped's parameter-expansion syntax or nullability suffixes (`@param ids -> (...)`, `:param!`, output aliases like `"name?"` / `"name!"`). `@name` is the borrowed annotation; parameter shape lives inline in the placeholder.
...
- Runtime-expanded params can appear only once per query for now. Reusing `:ids:list` twice would need duplicated driver args, so the parser rejects it clearly.
...
- Kept the vendored TypeSQL tree unchanged. Inline parameter modifiers are parsed and normalized before analysis; object/object-array expansions use representative named placeholders such as `:post__slug` so the analyzer can still infer SQLite column types.
