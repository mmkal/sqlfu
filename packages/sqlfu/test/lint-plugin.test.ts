...
test('generated-query-freshness: reports a missing generated wrapper for a query file', async () => {
  ...
  await using project = await makeAsyncProject({
    ...
    'sql/.generated/queries.ts': queriesManifest([{sqlFile: 'list-users.sql', sourceHash: sha256(querySql)}]),
  });
...
test('generated-query-freshness: reports a stale generated query manifest hash', async () => {
  const querySql = 'select id, name\nfrom users\norder by name;\n';
  await using project = await makeAsyncProject({
    'sqlfu.config.ts': `export default { queries: './sql' }`,
    'sql/list-users.sql': querySql,
    'sql/.generated/queries.ts': queriesManifest([{sqlFile: 'list-users.sql', sourceHash: '0'.repeat(64)}]),
...
test('generated-query-freshness: accepts a generated query manifest with a matching source hash and wrapper', async () => {
  const querySql = 'select id, name\nfrom users\norder by name;\n';
  await using project = await makeAsyncProject({
    'sqlfu.config.ts': `export default { queries: './sql' }`,
    'sql/list-users.sql': querySql,
...
function queriesManifest(entries: {sqlFile: string; sourceHash: string}[]): string {
  return [
    ...
    ...entries.map(
      (entry) =>
        `\t{ sqlFile: ${JSON.stringify(entry.sqlFile)}, generatedFile: ${JSON.stringify(`${entry.sqlFile}.ts`)}, sourceHash: ${JSON.stringify(entry.sourceHash)} },`,
    ),
    '];',
    '',
  ].join('\n');
